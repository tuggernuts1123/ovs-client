...
void eSettingsManager::Init()
{
    ...
    // Private Server
    char* serverUrlChar = ini.ReadString("Server.Game", "ServerUrl", "");
    if (serverUrlChar && serverUrlChar[0]) {
        int len = MultiByteToWideChar(CP_UTF8, 0, serverUrlChar, -1, nullptr, 0);
        if (len > 0) {
            szServerUrl.resize(len - 1);
            MultiByteToWideChar(CP_UTF8, 0, serverUrlChar, -1, &szServerUrl[0], len);
        }
    }

    char* prodServerUrlChar = ini.ReadString("Server.Prod", "ServerUrl", "");
    if (prodServerUrlChar && prodServerUrlChar[0]) {
        int len = MultiByteToWideChar(CP_UTF8, 0, prodServerUrlChar, -1, nullptr, 0);
        if (len > 0) {
            szProdServerUrl.resize(len - 1);
            MultiByteToWideChar(CP_UTF8, 0, prodServerUrlChar, -1, &szProdServerUrl[0], len);
        }
    }
