...
bool RunDumper()
{
    return RunDumper(HookMetadata::CurrentDllModule, false);
}

bool RunDumper(HMODULE hModule)
{
    return RunDumper(hModule, false);
}

bool RunDumper(HMODULE hModule, bool useExported)
{
    auto UnloadDumper = [](HMODULE hDumper)
        {
            if (hDumper)
            {
                FreeLibrary(hDumper);
                printfInfo("Unloaded Dumper DLL\n");
            }
        };

    bool returnCode = false;

    if (SettingsMgr->bLoadDumper)
    {
        HookMetadata::sActiveMods.bLoadDumper = true;
    }
    else {
        printfWarning("Dumper Loading Disabled in Settings. Skipping!\n");
        return returnCode;
    }

    if (SettingsMgr->sDumperDLLPath == "")
    {

        printfWarning("Dumper DLL Path is empty. Skipping!\n");
        return returnCode;
    }

    HMODULE hDumper = nullptr;
    HMODULE hAlreadyLoaded = GetModuleHandleA(SettingsMgr->sDumperDLLPath.c_str());

    if (hAlreadyLoaded)
    {
        printfWarning("Dumper DLL: %s is already loaded! Skipping LoadLibrary!\n", SettingsMgr->sDumperDLLPath.c_str());
        hDumper = hAlreadyLoaded;
    }
    else {
        printfInfo("Attempting to load Dumper DLL: %s\n", SettingsMgr->sDumperDLLPath.c_str());
        try
        {
            hDumper = LoadLibraryA(SettingsMgr->sDumperDLLPath.c_str());
        }
        catch (...)
        {
            printfError("Exception Occurred while loading Dumper DLL: %s\n", SettingsMgr->sDumperDLLPath.c_str());
            UnloadDumper(hDumper);
            return returnCode;
        }
    }

    try {
        printfSuccess("Loaded Dumper DLL: %s\n", SettingsMgr->sDumperDLLPath.c_str());
        printfYellow("Waiting for dumper to finish...\n");
        if (useExported)
        {
            typedef DWORD(__stdcall* ExternalDumper)(HMODULE);
            ExternalDumper DoDump = (ExternalDumper)GetProcAddress(hDumper, "RunDumper");

            if (!DoDump) {
                printfError("Failed to get RunDumper function from Dumper DLL: %s\n", SettingsMgr->sDumperDLLPath.c_str());
                UnloadDumper(hDumper);
                return returnCode;
            }

            DWORD dumpResult = DoDump(HookMetadata::CurrentDllModule);

            if (dumpResult == 0)
            {
                printfSuccess("Dumper Finished Successfully!\n");
                UnloadDumper(hDumper);
                return true;
            }
            else
            {
                printfError("Dumper Failed with error code: %d\n", dumpResult);
                UnloadDumper(hDumper);
                return returnCode;
            }
        }
    ...
    {
        printfError("Exception Occurred while calling Dumper DLL function: %s\n", SettingsMgr->sDumperDLLPath.c_str());
    }
}
